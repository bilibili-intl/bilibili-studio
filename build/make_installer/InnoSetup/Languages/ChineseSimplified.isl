...
KInstalling=正在安装
KInstallApp=%1 安装程序
DeleteProfile=同时删除配置文件吗?
CheckInValidFolder=检测到直播姬并未安装在Bilibili Studio或bililive文件夹，为了避免误删文件%n%n确定要删除目录 %1 下的所有程序数据吗?
WaitCurInstallFinish=请先等待%1的当前安装/卸载完成.
ApplicationRuningWhenUninstall=卸载程序检测到%1正在运行.%n%n您必须先关闭它然后单击 "是" 继续卸载, 或按 "否" 退出!
ApplicationRuningWhenInstall=安装程序检测到%1正在运行.%n%n您必须先关闭它然后单击 "是" 继续安装, 或按 "否" 退出!
IncompatibleVersion=检测到无法兼容的史前版本 %1 v%2%n%n请先手动卸载史前版本，再重试安装
HasHigerVersion=您已经安装更高版本的 %1 v%2
WindowsRunLevel=%1需要运行在windows 7及以上的操作系统
InvalidInstallPath=安装路径不合法!
CantIntsallInRootFolder=应用程序无法安装在根目录！
MayConflictWithOther=将直播姬安装在 bililive 或 Bilibili Studio 之外的目录中可能和其他程序产生冲突%n%n是否继续安装?