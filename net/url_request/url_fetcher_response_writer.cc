...
int URLFetcherStringWriter::Write(IOBuffer* buffer,
  ...
  {
	  LOG(ERROR) << "URLFetcherStringWriter::Write catch std::bad_alloc :" << ba.what();
  }
  catch (std::exception& ex)
  {
	  LOG(ERROR) << "URLFetcherStringWriter::Write catch std::exception : " << ex.what();
  }
  catch (...) {
	  LOG(ERROR) << "URLFetcherStringWriter::Write catch ...!";
  }
  return ERR_FAILED;
}
