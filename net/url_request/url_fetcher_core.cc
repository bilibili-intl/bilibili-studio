...
// static
base::ThreadLocalPointer<URLFetcherCore::Registry>
    URLFetcherCore::g_registry;
...
URLFetcherCore::URLFetcherCore(URLFetcher* fetcher,
    ...
    DCHECK(original_url_.is_valid()) << "raw string: " << original_url_.raw_string();
    LOG_IF(ERROR, !original_url_.is_valid()) << "raw string: " << original_url_.raw_string();
    // ]
}
...
void URLFetcherCore::StartURLRequest() {
  ...
  CurrentRegistry()->AddURLFetcherCore(this);
  current_response_bytes_ = 0;
...
// static
URLFetcherCore::Registry* URLFetcherCore::CurrentRegistry()
{
    if (!g_registry.Get()) {
        g_registry.Set(new Registry());
    }
