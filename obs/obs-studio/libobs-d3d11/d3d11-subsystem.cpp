...
void gs_device::InitDevice(uint32_t adapterIdx)
{
    ...
    //bililive [
    g_gpus_info_.cur_load_gpu_index = adapterIdx;
    // ]
    ...
    //bililive [
    //这里补条日志，查崩溃问题，看什么desc.VendorId等于多少会走到这下面
    blog(LOG_INFO, "desc.VendorId：%d", desc.VendorId);
    // ]
...
static inline void LogD3DAdapters()
{
    ...
    //bililive [
    g_gpus_info_.gpu_count = 0;
    g_gpus_info_.cur_load_gpu_index = 0;
    for (int i = 0; i < 4; i++)
    {
        memset(g_gpus_info_.gpus[i].gpu_name, 0, 512);
        memset(g_gpus_info_.gpus[i].driver_version, 0, 512);
    }
    // ]
	...
	for (i = 0; factory->EnumAdapters1(i, adapter.Assign()) == S_OK; ++i) {
        ...
        if (i < 4)
        {
	    g_gpus_info_.gpus[i].vendor_id = desc.VendorId;
	    g_gpus_info_.gpus[i].device_id = desc.DeviceId;

            strcpy(g_gpus_info_.gpus[i].gpu_name, name);
            if (SUCCEEDED(hr))
            {
                const uint64_t version = umd.QuadPart;
                const uint16_t aa = (version >> 48) & 0xffff;
                ...
                const uint16_t ccccc = (version >> 16) & 0xffff;
                const uint16_t ddddd = version & 0xffff;
                sprintf_s(g_gpus_info_.gpus[i].driver_version, "%d.%d.%d.%d",aa, bb, ccccc, ddddd);
            }
            g_gpus_info_.gpu_count++;
        }
        // ]
