...
static void winrt_capture_device_loss_rebuild(void *device_void, void *data)
{
    ...
    // bililive [
    typedef HRESULT(__stdcall * t_CreateDirect3D11DeviceFromDXGIDevice)(
        ::IDXGIDevice *, ::IInspectable **);
    t_CreateDirect3D11DeviceFromDXGIDevice MyCreateDirect3D11DeviceFromDXGIDevice = nullptr;
    HMODULE d3d11_module = ::GetModuleHandle(L"d3d11.dll");
    if (d3d11_module)
    {
        MyCreateDirect3D11DeviceFromDXGIDevice =
            (t_CreateDirect3D11DeviceFromDXGIDevice)GetProcAddress(d3d11_module, "CreateDirect3D11DeviceFromDXGIDevice");
    }

    if (!MyCreateDirect3D11DeviceFromDXGIDevice)
        return;
    // ]
    ...
    if (FAILED(MyCreateDirect3D11DeviceFromDXGIDevice(dxgi_device.Get(),
        inspectable.put())))
        blog(LOG_ERROR, "Failed to get WinRT device");
    // ]
	...
	try {
		...
		blog(LOG_ERROR, "CreateCaptureSession (0x%08X): %ls", err.code().value,
		     err.message().c_str());
		return;
	} catch (...) {
		blog(LOG_ERROR, "CreateCaptureSession (0x%08X)",
		     winrt::to_hresult().value);
		return;
	}
...
try {
	...
	// bililive [
	if (!d3d_device) {
		blog(LOG_ERROR, "Failed to get D3D device");
		return nullptr;
	}
	// ]
	...
	//					  inspectable.put());
    hr = MyCreateDirect3D11DeviceFromDXGIDevice(dxgi_device.Get(),
        inspectable.put());
    // ]
	...
	try {
		...
		blog(LOG_ERROR, "frame_pool.CreateCaptureSession (0x%08X): %ls",
		     err.code().value, err.message().c_str());
		return nullptr;
	} catch (...) {
		blog(LOG_ERROR, "frame_pool.CreateCaptureSession (0x%08X)",
		     winrt::to_hresult().value);
		return nullptr;
	}
	if (!session) {
...
extern "C" EXPORT void winrt_capture_thread_start()
{
	...
	try {
		...
		blog(LOG_ERROR, "winrt_capture_thread_start (0x%08X): %ls",
		     err.code().value,err.message().c_str());
	} catch (...) {
		blog(LOG_ERROR, "winrt_capture_thread_start (0x%08X)",
		     winrt::to_hresult().value);
	}	
}
