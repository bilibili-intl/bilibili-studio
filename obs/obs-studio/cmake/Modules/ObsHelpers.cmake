...
if(DEFINED ENV{obsAdditionalInstallFiles})
	...
	set(ENV{obsAdditionalInstallFiles} "${OBS_STUDIO_SOURCE_DIR}/additional_install_files")
	# ]
endif()
...
function(obs_helper_copy_dir target target_configs source dest)
	add_custom_command(TARGET ${target} POST_BUILD
		COMMAND "${CMAKE_COMMAND}"
			...
			-P "${OBS_STUDIO_SOURCE_DIR}/cmake/copy_helper.cmake"
			# ]
		VERBATIM)
endfunction()
...
function(obs_install_additional maintarget)
	...
	set(addfdir "${OBS_STUDIO_SOURCE_DIR}/additional_install_files")
	# ]
	if(DEFINED ENV{obsAdditionalInstallFiles})
...
function(obs_debug_copy_helper target dest)
	add_custom_command(TARGET ${target} POST_BUILD
		COMMAND "${CMAKE_COMMAND}"
			...
			-P "${OBS_STUDIO_SOURCE_DIR}/cmake/copy_on_debug_helper.cmake"
			# ]
		VERBATIM)
endfunction()
...
function(install_obs_pdb ttype target)
	...
	if("${ttype}" STREQUAL "PLUGIN")
		...
		if(CMAKE_SIZEOF_VOID_P EQUAL 8)
			#obs_debug_copy_helper(${target} "${BILILIVE_DEPLOYMENT_DIR}/x64/plugins")
            obs_debug_copy_helper(${target} "${BILILIVE_DEPLOYMENT_DIR}/plugins")
		else()
			obs_debug_copy_helper(${target} "${BILILIVE_DEPLOYMENT_DIR}/plugins")
		endif()
		# ]
		...
		obs_debug_copy_helper(${target} "${BILILIVE_DEPLOYMENT_DIR}")
		# ]
...
function(install_obs_plugin target)
	...
	add_custom_command(TARGET ${target} POST_BUILD
		COMMAND "${CMAKE_COMMAND}" -E copy
			...
			"${BILILIVE_DEPLOYMENT_DIR}/plugins/$<TARGET_FILE_NAME:${target}>"
			# ]
		VERBATIM)
...
function(install_obs_data target datadir datadest)
	...
	add_custom_command(TARGET ${target} POST_BUILD
		COMMAND "${CMAKE_COMMAND}" -E copy_directory
			...
			"${CMAKE_CURRENT_SOURCE_DIR}/${datadir}" "${BILILIVE_DEPLOYMENT_DIR}/data/${datadest}"
			# ]
		VERBATIM)
...
function(install_obs_data_from_abs_path target datadir datadest)
	...
	add_custom_command(TARGET ${target} POST_BUILD
		COMMAND "${CMAKE_COMMAND}" -E copy_directory
			...
			"${datadir}" "${BILILIVE_DEPLOYMENT_DIR}/data/${datadest}"
			# ]
		VERBATIM)
...
function(install_obs_data_file target datafile datadest)
	...
	add_custom_command(TARGET ${target} POST_BUILD
		COMMAND "${CMAKE_COMMAND}" -E make_directory
			...
			"${BILILIVE_DEPLOYMENT_DIR}/data/${datadest}"
			# ]
		VERBATIM)
	add_custom_command(TARGET ${target} POST_BUILD
		COMMAND "${CMAKE_COMMAND}" -E copy
			...
			"${CMAKE_CURRENT_SOURCE_DIR}/${datafile}" "${BILILIVE_DEPLOYMENT_DIR}/data/${datadest}"
			# ]
		VERBATIM)
...
function(install_obs_datatarget target datadest)
	...
	add_custom_command(TARGET ${target} POST_BUILD
		COMMAND "${CMAKE_COMMAND}" -E copy
			...
			"${BILILIVE_DEPLOYMENT_DIR}/data/${datadest}/$<TARGET_FILE_NAME:${target}>"
			# ]
		VERBATIM)
	...
	# modify [ datatarget also need pick .pdb
	obs_debug_copy_helper(${target} "${BILILIVE_DEPLOYMENT_DIR}/data/${datadest}")
	# ]
endfunction()
