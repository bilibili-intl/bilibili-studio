...
void WASAPISource::InitClient()
{
        ...
        //	throw HRError("Failed to get initialize audio client", res);
    {
        if (settings_)
        {
            obs_data_set_int(settings_, "audio_render_error", 1);
        }
        throw HRError("Failed to get initialize audio client", res);
    }
    else
    {
        if (settings_)
        {
            obs_data_set_int(settings_, "audio_render_error", 0);
        }
    }
    // ]
}
...
void WASAPISource::InitCapture()
{
	...
	if (settings_)
	{
	    obs_data_set_string(settings_, "audio_device_name", device_name.c_str());
	}
	// ]
