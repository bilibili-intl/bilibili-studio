		       ...
		       EnumDeviceCallback callback, void *param)
{
    ...
    // ]
    // bililive [
    //避免CheckForDecklinkVideo的时候把摄像头BindToObject导致内存泄露
    //只有CheckForDecklinkVideo调EnumDevice的时候param为NULL
    if (param != nullptr || wcsstr(deviceName.bstrVal, L"Decklink") != nullptr)
    {
        hr = deviceInfo->BindToObject(NULL, 0, IID_IBaseFilter,
            (void**)&filter);
    }
