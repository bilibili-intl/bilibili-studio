...
#include <atlcomcli.h>
#include "libdshowcapture/source/ComPtr.hpp"
#include "libdshowcapture/source/dshow-base.hpp"
#include <mutex>
...
enum UpdateType {
    ...
    UT_DEFAULT        = 1 << 0,
    UT_CHECK          = 1 << 1,
    UT_CALIBRATE_HEAD = 1 << 2,
    UT_CALIBRATE_FACE = 1 << 3,
};
...
// bililive [
#define AUTO_RETRY        "retry_when_failed_to_active"
#define RETRY_INTERVAL_MS "retry_interval_ms"

static const char kCameraSignal[] = "camera_signal";
static const char kCameratatus[] = "camera_status";

static const char kCameraOccupiedSignal[] = "camera_occupied_signal";
static const char kCameraOccupiedStatus[] = "camera_occupied_status";
// ]
...
// bililive [
static bool g_hide_camera_frame_ = false;
...
struct DShowInput {
    ...
    // bililive [
    bool should_retry_when_failed;
    int retry_interval;
    UINT_PTR retry_timer_id;
    PropertiesData properties_data_;
    std::vector<std::pair<std::wstring, std::wstring>> device_list_;
    std::mutex enum_mutex_;
    // ]
	...
	// bililive [
	bool DeviceChanged(std::vector<std::pair<std::wstring, std::wstring>>& device);
	// ]
};
...
// bililve [
static inline void ProcessMessages(DShowInput* dinput)
// ]
{
	MSG msg;
	while (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
        ...
        if (msg.message == WM_TIMER)
        {
            ...
            if (msg.wParam == dinput->retry_timer_id)
                dinput->QueueAction(Action::Activate);
        }
        else
        // ]
		    DispatchMessage(&msg);
...
enum ErrorImageType
{
    OPEN_FAILED = 0, //摄像头打开失败
    IS_RADIO         //电台分区
};
static inline bool SetErrorImage(obs_source_t* source, ErrorImageType type)
{
    char* fail_image_file = nullptr;
    if (type == OPEN_FAILED)
    {
        fail_image_file = obs_module_file("open_camera_failed.png");
    }
    else if (type == IS_RADIO)
    {
        fail_image_file = obs_module_file("is_radio.png");
    }

    if (!fail_image_file)
        return false;

    uint32_t fail_image_width, fail_image_height;
    gs_color_format fail_image_format;
    uint8_t* fail_image_data = gs_create_texture_file_data(fail_image_file,
        &fail_image_format,
    ...
    video_format fail_frame_format;
    switch (fail_image_format)
    {
    case GS_RGBA:
        fail_frame_format = VIDEO_FORMAT_RGBA;
        break;
    case GS_BGRX:
        fail_frame_format = VIDEO_FORMAT_BGRX;
        break;
    case GS_BGRA:
        fail_frame_format = VIDEO_FORMAT_BGRA;
        break;
    default:
        fail_frame_format = VIDEO_FORMAT_NONE;
        blog(LOG_ERROR, "[win-dshow] Unsupport color format for fail_image");
        break;
    }

    if (fail_frame_format != VIDEO_FORMAT_NONE)
    {
        obs_source_frame* frame = obs_source_frame_create(fail_frame_format, fail_image_width, fail_image_height);
        memcpy(frame->data[0], fail_image_data, fail_image_width * fail_image_height * 4);
        obs_source_output_video(source, frame);
        obs_source_frame_destroy(frame);
    }

    bfree(fail_image_data);

    return fail_frame_format != VIDEO_FORMAT_NONE;
}
// ]
	...
	while (true) {
		...
		case Action::ActivateBlock: {
			...
			// bililive [
                        ...
                        if (settings) video_device_id = obs_data_get_string(settings, VIDEO_DEVICE_ID);

                        bool use_default_error_output = true;

                        if (!video_device_id.empty())
                        {
		            if (SetErrorImage(source, ErrorImageType::OPEN_FAILED))
		            	use_default_error_output = false;

                            if (should_retry_when_failed)
                                retry_timer_id = SetTimer(NULL, retry_timer_id, retry_interval, nullptr);
                        }

                        if (use_default_error_output)
                            obs_source_output_video2(source, nullptr);
            	        // ]
		    }
...
{
    // bililive [
    if (true == g_hide_camera_frame_)
    {
        if (SetErrorImage(source, ErrorImageType::IS_RADIO))
            return;
...
{
    // bililive [
    //加锁了，防止UpdateVideoConfig和GetDShowProperties多线程访问
    std::lock_guard<std::mutex> lock(enum_mutex_);
    // ]
    ...
    // bililive [
    should_retry_when_failed = obs_data_get_bool(settings, AUTO_RETRY);
    retry_interval = (int)obs_data_get_int(settings, RETRY_INTERVAL_MS);
    // ]
	DeviceId id;
	...
	// ]
    // bililive [
    //这里我们使用保存的成员变量properties_data_，不需要重新Device::EnumVideoDevices
    //防止遇到无他等摄像头造成内存泄露
    if (DeviceChanged(device_list_))
    {
        Device::EnumVideoDevices(properties_data_.devices);
    }
    VideoDevice dev;
    if (!properties_data_.GetDevice(dev, video_device_id.c_str())) {
		blog(LOG_WARNING, "%s: properties_data_.GetDevice failed",
			obs_source_get_name(source));
		return false;
	}
    // ]
...
// bililive [
bool DShowInput::DeviceChanged(std::vector<std::pair<std::wstring, std::wstring>>& device)
{
    //重新获取一次设备列表

    //代码来自于 Device::EnumVideoDevices 并且展开
    //因为原代码调用 BindToStorage 和 BindToObject 是连续调用的并且没有提供跳过的逻辑
    //但 BindToObject 在无他相机上会造成内存泄漏
    std::vector<std::pair<std::wstring, std::wstring>> device_list;
    ComPtr<ICreateDevEnum> create_dev_enum;
    HRESULT hr = CoCreateInstance(CLSID_SystemDeviceEnum, NULL, CLSCTX_INPROC_SERVER,
        IID_ICreateDevEnum, (void **)&create_dev_enum);
    if (S_OK != hr)
        return true;
    ...
    while (true)
    {
        ...
        if (enum_moniker->Next(1, &moniker, &fetch) != S_OK)
        {
            break;
        }

        //获取当前摄像头的名字和路径
        ComPtr<IPropertyBag> property_bag;
        hr = moniker->BindToStorage(0, 0, IID_IPropertyBag, (void**)&property_bag);
        if (S_OK != hr)
        {
            continue;
        }
        std::wstring friendly_name = L"";
        std::wstring device_path = L"";

        CComVariant deviceName, devicePath;
        deviceName.vt = VT_BSTR;
        deviceName.bstrVal = NULL;
        devicePath.vt = VT_BSTR;
        devicePath.bstrVal = NULL;
        hr = property_bag->Read(L"FriendlyName", &deviceName, NULL);
        if (S_OK != hr)
        {
            continue;
        }
        friendly_name = deviceName.bstrVal;
        if (deviceName.bstrVal &&
            wcsstr(deviceName.bstrVal, L"Decklink") != nullptr)
        {
            continue;
        }
        hr = property_bag->Read(L"DevicePath", &devicePath, NULL);
        if (S_OK == hr)
        {
            device_path = devicePath.bstrVal;
        }

        device_list.push_back(std::make_pair(friendly_name, device_path));
    }

    //判断和前一次获取的列表有无不同
    if (device_list.size() == device.size() && std::equal(device_list.begin(), device_list.end(), device.begin()))
    {
        return false;
    }
    else
...
// bililive [
void SetObsCameraFrameHideStatus(bool status)
{
    g_hide_camera_frame_ = status;
}
// ]
...
static void GetDShowDefaults(obs_data_t *settings)
{
    ...
    // bililive [
    obs_data_set_default_bool(settings, AUTO_RETRY, true);
    obs_data_set_bool(settings, AUTO_RETRY, true);
    obs_data_set_default_int(settings, RETRY_INTERVAL_MS, 3000);
    obs_data_set_int(settings, RETRY_INTERVAL_MS, 3000);
    obs_data_set_bool(settings, DEACTIVATE_WNS, true);
    // ]
}
...
static obs_properties_t *GetDShowProperties(void *obj)
{
    ...
    // ]
    // bililive [
    // 只有设备发生变化的情况下才会重新EnumVideoDevices更新设备
    if (input->DeviceChanged(input->device_list_))
    {
        Device::EnumVideoDevices(data->devices);
        input->properties_data_.devices = data->devices;
    }
    else
    {
        data->devices = input->properties_data_.devices;
    }
    // ]
