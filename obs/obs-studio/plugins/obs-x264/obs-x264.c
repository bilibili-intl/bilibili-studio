...
//禁用AVX2指令集，目前已知i3-4000m不支持这个指令集，不禁用会崩溃
static bool need_disable_avx2()
{
    ...
    if (cached_result < 0)
    {
        ...
        if (status == ERROR_SUCCESS)
        {
            ...
            if (status == ERROR_SUCCESS)
            {
                //全部转换为小写
                int index = 0;
                while (data[index] != L'\0')
                {
                    wchar_t c = data[index];
                    data[index] = towlower(c);
                    index++;
                }
                if (wcsstr(data, L"i3-4000m") != NULL ||
                    wcsstr(data, L"i3-4100m") != NULL)
                {
                    ret = true;
                }
            }
			  ...
			  const struct obs_x264_options *options, bool update)
{
	...
	//bilibili add[
	if (obsx264->params.i_fps_num == 0 || obsx264->params.i_fps_den == 0)
	{
		obsx264->params.i_fps_num = 30;
		obsx264->params.i_fps_den = 1;
	}
	//]
    ...
    // bililive [
#ifdef _MSC_VER
    if (need_disable_avx2())
    {
        obsx264->params.cpu &= ~X264_CPU_AVX2;
    }
#endif
    // ]
			    ...
			    bool *received_packet)
{
	...
	*received_packet = (nal_count != 0);
    ...
    if (*received_packet)
    {
        const char* frame_type = "P";
        if (IS_X264_TYPE_B(pic_out.i_type))
        {
            frame_type = "B";
        }
        else if (IS_X264_TYPE_I(pic_out.i_type))
        {
            frame_type = "I";
        }
        int qp = pic_out.i_qpplus1 - 1;

        signal_handler_t* encoder_signal = obs_encoder_get_signal_handler(obsx264->encoder);
        if (encoder_signal)
        {
            calldata_t calldata;
            calldata_init(&calldata);
            calldata_set_ptr(&calldata, "encoder", obsx264->encoder);
            calldata_set_string(&calldata, "frame_type", frame_type);
            calldata_set_float(&calldata, "qp", (double) qp);
            signal_handler_signal(encoder_signal, "encoded_qp", &calldata);
