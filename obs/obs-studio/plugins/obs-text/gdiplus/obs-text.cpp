...
// Special thanks to http://stackoverflow.com/a/1031773/1746503.
bool is_utf8_encoded(const char* string)
{
    if (!string)
        return false;
...
void TextSource::LoadFileText()
{
    ...
    // bililive [
        ...
        int text_count = MultiByteToWideChar(CP_ACP, 0, file_text, -1, nullptr, 0) - 1;
        if (text_count > 0) {
            text.resize(text_count);
            MultiByteToWideChar(CP_ACP, 0, file_text, -1, &text[0], text_count);
        }
#endif
    }
    // ]
