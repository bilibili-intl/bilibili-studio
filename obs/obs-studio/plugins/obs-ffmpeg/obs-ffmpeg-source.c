...
// bililive [
static const char kMediaCallback[] = "media_callback";
static const char kMediaState[] = "media_state";
// ]
...
// bililive [
static void send_media_state(obs_source_t *source,
	                     enum obs_media_state state)
{
	...
	if (signal) {
		...
		calldata_set_int(&calldata, kMediaState, state);
		signal_handler_signal(signal, kMediaCallback, &calldata);
...
static void set_media_state(void *data, enum obs_media_state state)
{
	...
	// bililive [
	send_media_state(s->source, state);
	// ]
}
