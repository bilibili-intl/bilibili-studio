			 ...
			 struct encoder_packet *packet, bool *received_packet)
{
    ...
    if (*received_packet)
    {
        uint8_t *sd = av_packet_get_side_data(&av_pkt, AV_PKT_DATA_QUALITY_STATS, NULL);

        if (sd)
        {
            double quantization_parameter = AV_RL32(sd) / (double)FF_QP2LAMBDA;
            int n_frame_type = sd[4];
            const char* frame_type = 0;

            if (n_frame_type == AV_PICTURE_TYPE_I)
            {
                frame_type = "I";
            }
            else if (n_frame_type == AV_PICTURE_TYPE_P)
            {
                frame_type = "P";
            }
            else if (n_frame_type == AV_PICTURE_TYPE_B)
            {
                frame_type = "B";
            }

            signal_handler_t* encoder_signal = obs_encoder_get_signal_handler(enc->encoder);
            if (encoder_signal)
            {
                calldata_t calldata;
                calldata_init(&calldata);
                calldata_set_ptr(&calldata, "encoder", enc->encoder);
                calldata_set_string(&calldata, "frame_type", frame_type);
                calldata_set_float(&calldata, "qp", quantization_parameter);
