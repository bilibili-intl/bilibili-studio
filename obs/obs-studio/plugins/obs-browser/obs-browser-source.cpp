...
{
    // bililive [
    blog(LOG_INFO, "obs-browser destructing.");
    // ]
    ...
    *p_prev_next = next;
    ...
    // bililive [
    blog(LOG_INFO, "obs-browser destructed.");
    // ]
}

// bililive [
bool BrowserSource::ExecuteOnBrowser(BrowserFunc func, bool async)
// ]
{
	// bililive [
    bool success = false;
...
#else
			// bililive [
			success = QueueCEFTask([=]() { func(browser); });
			// ]
#endif
		}
    }
...
bool BrowserSource::CreateBrowser()
{
	return QueueCEFTask([this]() {
		...
		if (is_local) {
			...
			//cefBrowserSettings.web_security = STATE_DISABLED;  //DISCARD
		}
#endif
...
void BrowserSource::DestroyBrowser(bool async)
{
    ...
    if (cefBrowser)
    {
        blog(LOG_INFO, "obs-browser destroying %s", async ? "true" : "false");
    }
    bool success = ExecuteOnBrowser(
    // ]
    ...
    if (cefBrowser)
    {
        ...
        if (bc)
        {
            // 不管上面的任务投递是否成功，这里都把cefclient和browsersource解绑，防止：
            // 1、投递成功了，但是任务延迟执行，导致执行时browsersource已经析构而引发野指针崩溃；
            // 2、投递失败了，browsersource析构后cefclient引用到野指针；
            bc->DetachObsBrowserSource();

            // 任务投递失败了，在这里直接关闭cefclient
            if (!success)
            {
                blog(LOG_ERROR, "obs-browser async close browser failed, try sync close");
        ...
        {
            blog(LOG_ERROR, "obs-browser get empty client after excute destroy %s", async ? "true" : "false");
        }
    }
    // ]
				   ...
				   int x_delta, int y_delta)
{
	...
	if (x_delta == 0 && y_delta == 0) {
		blog(LOG_ERROR, "[obs-browser]: SendMouseWheel x_delta and y_delta is invalid");
		return;
	}
	// ]
...
void BrowserSource::SendFocus(bool focus)
{
	ExecuteOnBrowser(
		[=](CefRefPtr<CefBrowser> cefBrowser) {
			//cefBrowser->GetHost()->SendFocusEvent(focus);  //DISCARD
...
void BrowserSource::Update(obs_data_t *settings)
{
	if (settings) {
		...
		blog(LOG_INFO, "[obs-browser]: '%s' url:\n\t%s",
		     name, url.c_str());
		// ]
	}
