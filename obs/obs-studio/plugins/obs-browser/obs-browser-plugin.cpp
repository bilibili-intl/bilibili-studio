...
{
    // bililive[
    bool ret = CefPostTask(TID_UI,
			   CefRefPtr<BrowserTask>(new BrowserTask(task)));
    if (!ret)
    {
        blog(LOG_ERROR, "obs-browser CefPostTask failed!");
    }
    return ret;
    // ]
}
...
// bililive [
static const char *bililive_default_css = "\
body { \
background-color: rgba(0, 0, 0, 0); \
margin: 0px auto; \
overflow: visible; \
...
static void browser_source_get_defaults(obs_data_t *settings)
{
    ...
    obs_data_set_default_string(settings, "css", bililive_default_css);

    // 区分业务类型
    obs_data_set_default_int(settings, "activity_type", 0);
    obs_data_set_default_string(settings, "activity_id", "");
...
{
    // bililive[
    // cef模块的render进程采用我们自己的bililive_browser进程，
    // cef模块的初始化也走的我们的自己的cef_proxy::cef_proxy_init_module，
    // 但需要注意我们的cef_proxy_dll必须先于obs加载，否则obs加载之后可能引发崩溃
    return;
    // ]
...
void obs_module_unload(void)
{
    ...
    if (manager_thread.joinable()) {
        ...
        // cef模块的render进程采用我们自己的bililive_browser进程，
        // cef模块的初始化也走的我们的自己的cef_proxy::cef_proxy_init_module，
        // 自然cef模块的停止也由我们自己的cef_proxy::cef_proxy_shutdown_module来完成，
        // 不需要由obs的browser-plugin来做这个事情，
        // 但需要注意我们的cef_proxy_dll必须先于obs加载，否则obs加载之后可能引发崩溃
