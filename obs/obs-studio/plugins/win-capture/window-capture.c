...
struct window_capture {
	...
	// bililive [
	bool is_capture_from_whole_screen;
	// ]
...
static void update_settings(struct window_capture *wc, obs_data_t *s)
{
	...
	// bililive [
	wc->is_capture_from_whole_screen = obs_data_get_bool(s, "capture_from_whole_screen");
	// ]

	// bililive [
	wc->use_hwnd_spesc = obs_data_get_bool(s, "use_hwnd_spesc");
	wc->hwnd_spesc = obs_data_get_int(s, "hwnd_spesc");
	// ]
...
static void wc_update(void *data, obs_data_t *settings)
{
	...
	// bililive [
	if (wc->use_hwnd_spesc)
	{
		wc->method = METHOD_BITBLT;
		wc->window = wc->hwnd_spesc;
	}
	// ]
}
...
static void wc_defaults(obs_data_t *defaults)
{
	...
	// bililive [
	obs_data_set_default_bool(defaults, "use_hwnd_spesc", false);
	obs_data_set_default_int(defaults, "hwnd_spesc", 0);
	// ]
}
...
static void wc_tick(void *data, float seconds)
{
	...
	if (!wc->window || !IsWindow(wc->window)) {
		...
		reset_capture = true;
		// bililive [
	} else if (IsIconic(wc->window) ||(wc->use_hwnd_spesc && !IsWindowVisible(wc->window))) {
		// ]bililive
		return;
	}
	...
	if (wc->method == METHOD_BITBLT) {
		...
		// bililive [
		dc_capture_capture(&wc->capture, wc->window, wc->is_capture_from_whole_screen);
		// ]
