...
// bililive [
#define SUPPRESSION_MARK_NAME L"bililive_window_capture_disable_performance_mode"
// ]
...
bool obs_module_load(void)
{
    ...
    // bililive [
    HANDLE suppression_mark = OpenMutexW(MUTEX_MODIFY_STATE, FALSE, SUPPRESSION_MARK_NAME);
    // ]

	obs_enter_graphics();

    // bililive [
    // Allow use of DC-copy mode to prevent displaying a black window problem.
	if (!suppression_mark && win8_or_above && gs_get_device_type() == GS_DEVICE_DIRECT3D_11) {
		obs_register_source(&duplicator_capture_info);
	} else {
		obs_register_source(&monitor_capture_info);
		CloseHandle(suppression_mark);
    }
	// ]
