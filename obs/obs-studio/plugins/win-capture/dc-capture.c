...
// bililive [
static void ClientRectToScreen(HWND hwnd, RECT* rect)
{
	POINT a, b;
	a.x = rect->left;
	a.y = rect->top;
...
// bililive [
void dc_capture_capture(struct dc_capture *capture, HWND window, bool from_screen_dc)
// ]
{
	HDC hdc_target;
	HDC hdc;
...
// bililive [
	...
	else
	{
		...
		ClientRectToScreen(window, &winRect);
		BitBlt(hdc, 0, 0, capture->width, capture->height,
			hdc_target, capture->x + winRect.left, capture->y + winRect.top, SRCCOPY);
