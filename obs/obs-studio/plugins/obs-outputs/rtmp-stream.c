...
// bililive [
__declspec(thread) struct rtmp_stream* current_stream = 0;

static const char rtmp_fmt_begin[] = "=RTMP_ERRFMT_BEGIN=";
static const char rtmp_fmt_end[] = "=RTMP_ERRFMT_END=";
static const char rtmp_err_begin[] = "=RTMP_ERRMSG_BEGIN=";
static const char rtmp_err_end[] = "=RTMP_ERRMSG_END=";

static  int current_count = 0; //统计发包失败次数(3次上报)

static void append_rtmp_error_to_output_error(struct rtmp_stream* stream)
{
    char* newmsg = 0;
    const char* output_last_error;
    int msglen;

    output_last_error = obs_output_get_last_error(stream->output);
    if (!output_last_error)
        output_last_error = "";

    msglen = strlen(output_last_error)
        + strlen(stream->last_rtmp_error_format)
        + strlen(stream->last_rtmp_error_message)
        + sizeof(rtmp_fmt_begin) + sizeof(rtmp_fmt_end) - 2
        + sizeof(rtmp_err_begin) + sizeof(rtmp_err_end) - 2
        + 1;
    newmsg = (char*)calloc(msglen, sizeof(char));
    strcat_s(newmsg, msglen, output_last_error);
    strcat_s(newmsg, msglen, rtmp_fmt_begin);
    strcat_s(newmsg, msglen, stream->last_rtmp_error_format);
    strcat_s(newmsg, msglen, rtmp_fmt_end);
    strcat_s(newmsg, msglen, rtmp_err_begin);
    strcat_s(newmsg, msglen, stream->last_rtmp_error_message);
    strcat_s(newmsg, msglen, rtmp_err_end);

    obs_output_set_last_error(stream->output, newmsg);
    free(newmsg);
}
// ]
...
static void log_rtmp(int level, const char *format, va_list args)
{
	...
	if (current_stream)
	{
		strcpy_s(current_stream->last_rtmp_error_format,
			 sizeof(current_stream->last_rtmp_error_format),
			 format);
		vsprintf_s(current_stream->last_rtmp_error_message,
			   sizeof(current_stream->last_rtmp_error_message),
			   format, args);
	}
	// ]
...
static void rtmp_stream_stop(void *data, uint64_t ts)
{
	...
	if (settings)
	{
		obs_data_set_int(settings, "encoder_generated_bytes", stream->encoder_generated_bytes);
		obs_data_set_int(settings, "drop_filler_bytes", stream->drop_filler_bytes);
...
static void *send_thread(void *data)
{
	...
	//bililive[
	if ( current_count > 2) {
	    obs_output_post_send_fail(stream->output);
	     //上一次结束，下一次开始
	    current_count = 0;
	}
	//]
...
static int try_connect(struct rtmp_stream *stream)
{
	...
	if (!RTMP_Connect(&stream->rtmp, NULL)) {
		set_output_error(stream);
		// bililive [
		append_rtmp_error_to_output_error(stream);
		// ]
...
static bool rtmp_stream_start(void *data)
{
	...
	// bililive[
	memset(stream->last_rtmp_error_format, 0,
	       sizeof(stream->last_rtmp_error_message));
	memset(stream->last_rtmp_error_message, 0,
	       sizeof(stream->last_rtmp_error_message));

	stream->encoder_generated_bytes = 0;
	stream->drop_filler_bytes = 0;
	// ]
			...
			int highest_priority, bool pframes)
{
    ...
    //bililive [
    stream->drop_frame_bytes += dropped_bytes;
    // ]
#ifdef _DEBUG
...
static void rtmp_stream_data(void *data, struct encoder_packet *packet)
{
	...
	if (packet->type == OBS_ENCODER_VIDEO) {
		...
		// bililive [
		stream->encoder_generated_bytes += packet->size;
		if (packet->size > new_packet.size)
			stream->drop_filler_bytes += packet->size - new_packet.size;
		// ]
	} else {
...
//bililive [
static uint64_t rtmp_stream_dropped_frames_bytes(void *data)
{
    struct rtmp_stream *stream = data;
    return stream->drop_frame_bytes;
}
//]
...
struct obs_output_info rtmp_output_info = {
	...
	// bililive [
    .get_dropped_frames_bytes = rtmp_stream_dropped_frames_bytes
	// ]
};
