...
static const char *output_signals[] = {
	...
	"void post_send_fail(ptr output)",
	NULL,
};
...
//bililive [
struct streaming_info obs_output_get_streaming_info(struct obs_output *output)
{
    struct streaming_info info;
    struct obs_core_video *video = &obs->video;

    if (video)
    {
        info.drawn = video->total_frames - output->starting_drawn_count;
        info.lagged = video->lagged_frames - output->starting_lagged_count;
        info.dropped_frame = obs_output_get_frames_dropped(output);
        info.total_frame = output->total_frames;
        info.total_bytes = obs_output_get_total_bytes(output);
        info.skipped = video_output_get_skipped_frames(video->video);
        info.dropped_frame_bytes = obs_output_get_frames_dropped_bytes(output);
    }
    return info;
}
// ]
...
//bililive [
int obs_output_get_frames_dropped_bytes(const obs_output_t *output)
{
    if (!obs_output_valid(output, "obs_output_get_frames_dropped_bytes"))
        return 0;
    if (!output->info.get_dropped_frames_bytes)
        return 0;

    return output->info.get_dropped_frames_bytes(output->context.data);
}
// ]
...
// bililive[
static inline void signal_post_send(struct obs_output* output)
{
   do_output_signal(output, "post_send_fail");
}
//]
...
void  obs_output_post_send_fail(struct obs_output* output)
{
	signal_post_send(output);
