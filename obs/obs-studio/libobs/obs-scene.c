...
static void scene_load_item(struct obs_scene *scene, obs_data_t *item_data)
{
    ...
    // bililive [
    bool fit_to_screen = obs_data_get_bool(item_data, "fit_to_screen");
    obs_sceneitem_set_fit_to_screen(item, fit_to_screen);
    // ]
...
// bililive [
bool obs_sceneitem_fit_to_screen(const obs_sceneitem_t *item)
{
    return item ? item->fit_to_screen : false;
}

bool obs_sceneitem_set_fit_to_screen(obs_sceneitem_t *item, bool fit_to_screen)
{
    if (!item)
        return false;

    if (item->fit_to_screen == fit_to_screen)
        return false;

    if (!item->parent)
        return false;

    item->fit_to_screen = fit_to_screen;
