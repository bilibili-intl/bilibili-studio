...
// bililive [
// Change the directory pathes below to adapt to bililive hierarchy.
// Assume that the current directory is in the [version]/.
...
static const char *module_data[] = {"data/plugins/%module%"};

cpu_and_os_version g_system_info_;
EXPORT struct cpu_and_os_version obs_get_cpu_and_os_version()
{
    return g_system_info_;
}
// ]
...
static void log_processor_info(void)
{
	...
	if (status == ERROR_SUCCESS) {
        ...
        //bililive [
        memset(g_system_info_.cpu_name, 0, 512);
        strcpy(g_system_info_.cpu_name, str);
        // ]
		bfree(str);
	}
...
static void log_windows_version(void)
{
    ...
    //bililive [
    memset(g_system_info_.os_version, 0, 512);
    sprintf(g_system_info_.os_version, "Windows Version:%d.%d Build %d (release: %s; revision: %d; %s-bit)",
        ver.major, ver.minor, ver.build, release_id, ver.revis,
        windows_bitness);
    // ]
}
...
static void log_aero(void)
{
    ...
    //bililive [
    g_system_info_.aero = bComposition;
    // ]
}
...
static void log_security_products_by_type(IWSCProductList *prod_list, int type)
{
	...
	for (int i = 0; i < count; i++) {
		...
		char *str = NULL;
		os_wcs_to_utf8_ptr(name, 0, &str);
		blog(LOG_INFO, "\t%s: %s (%s)", str,
		// ]
