...
// bililive [
static const char *encoder_signals[] = {
    "void encoded_qp(ptr encoder, string frame_type, int qp)",
    NULL
};
// ]
			 ...
			 obs_data_t *settings, obs_data_t *hotkey_data)
{
	...
	// bililive [
	pthread_mutex_init_value(&encoder->pkt_filter_mutex);
	// ]
    ...
    // bililive [
	if (pthread_mutex_init(&encoder->pkt_filter_mutex, &attr) != 0)
		return false;
    signal_handler_add_array(encoder->context.signals, encoder_signals);
    // ]
...
static void obs_encoder_actually_destroy(obs_encoder_t *encoder)
{
	if (encoder) {
		...
		// bililive [
		pthread_mutex_lock(&encoder->pkt_filter_mutex);
		for (size_t i = 0; i < encoder->pkt_filter.num; i++)
		{
			if (encoder->pkt_filter.array[i])
			{
				encoder->pkt_filter_info.array[i]->release(encoder->pkt_filter.array[i]);
		        }
		}
		da_free(encoder->pkt_filter);
		da_free(encoder->pkt_filter_info);
		pthread_mutex_unlock(&encoder->pkt_filter_mutex);
		pthread_mutex_destroy(&encoder->pkt_filter_mutex);
		// ]
			     ...
			     bool received, struct encoder_packet *pkt)
{
	...
	if (received) {
		...
		for (size_t i = 0; i < encoder->pkt_filter.num; i++)
		{
			if (encoder->pkt_filter.array[i])
			{
				pkt = encoder->pkt_filter_info.array[i]->filter(
					encoder->pkt_filter.array[i], encoder, pkt);
			}
		}
...
signal_handler_t* obs_encoder_get_signal_handler(const obs_encoder_t* encoder)
{
    return obs_encoder_valid("encoder", "obs_encoder_get_signal_handler")
        ? encoder->context.signals : 0;
}

void obs_encoder_set_packet_filter(obs_encoder_t* encoder, struct encoder_packet_filter_info* filter_info, void* ctx)
{
	if (!obs_encoder_valid(encoder, "obs_encoder_set_packet_filter"))
		return;

	if (!filter_info || !ctx)
		return;
	pthread_mutex_lock(&encoder->pkt_filter_mutex);
	da_push_back(encoder->pkt_filter_info, &filter_info);
	da_push_back(encoder->pkt_filter, &ctx);
	pthread_mutex_unlock(&encoder->pkt_filter_mutex);
}

int64_t obs_encoder_pkt_ts_to_frame_ts(obs_encoder_t* encoder, int64_t pkt_ts)
{
	if (!encoder->first_received)
		return -1;
	if (!encoder->start_ts) // 和 video_frame 的 timestamp 单位一致，每帧增加 (uint64_t)(1000000000.0 * (double)info->fps_den / (double)info->fps_num)
		return -1;
	return encoder->start_ts + pkt_ts * (uint64_t)(1000000000.0 * (double)encoder->timebase_num / (double)encoder->timebase_den);
}
// ]
