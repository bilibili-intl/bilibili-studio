...
typedef struct
{
    void* (*as_type)(int type, void* ctx);
    void (*destroy)(void* ctx);
    void* (*copy)(void* ctx);
} obs_source_frame_extra_op;
...
struct obs_source_frame {
    ...
    // bililive [
    void                       *extra_data;
    obs_source_frame_extra_op  *extra_data_op;
    // ]
};
...
typedef void(*deferred_updated_callback)(void*);

EXPORT bool obs_source_is_deferred_updated_handler_installed(obs_source_t* source);

EXPORT void obs_source_install_deferred_updated_handler(obs_source_t* source,
                                                        deferred_updated_callback handler,
                                                        void* param);

EXPORT void obs_source_uninstall_deferred_updated_handler(obs_source_t* source);
...
// bililive [
EXPORT bool obs_sceneitem_fit_to_screen(const obs_sceneitem_t *item);
EXPORT bool obs_sceneitem_set_fit_to_screen(obs_sceneitem_t *item, bool fit_to_screen);
// ]
...
//bililive [
EXPORT int obs_output_get_frames_dropped_bytes(const obs_output_t *output);
// ]
...
static inline void obs_source_frame_free(struct obs_source_frame *frame)
{
	if (frame) {
        ...
        if (frame->extra_data && frame->extra_data_op)
            (*frame->extra_data_op->destroy)(frame->extra_data);
        // ]
		bfree(frame->data[0]);
...
// bililive [
EXPORT signal_handler_t* obs_encoder_get_signal_handler(const obs_encoder_t* encoder);

typedef struct encoder_packet_filter_info
{
    struct encoder_packet* (*filter)(void* ctx, obs_encoder_t* encoder, struct encoder_packet* pkt);
    void (*release)(void* ctx);
} encoder_packet_filter_info;

EXPORT int64_t obs_encoder_pkt_ts_to_frame_ts(obs_encoder_t* encoder, int64_t pkt_ts);
EXPORT void obs_encoder_set_packet_filter(obs_encoder_t* encoder, struct encoder_packet_filter_info* hook_info, void* ctx);

// don't release! don't hold! don't use async! use in video thread! or crash!
EXPORT struct obs_source_frame* obs_source_peek_frame(obs_source_t* source);

struct streaming_info
{
    //如果drawn 和 lagged都大于0，表示渲染丢帧，lagged是丢帧帧数，和total_frame可以一起计算渲染丢帧率
    uint32_t drawn; //obs_core_video渲染帧数减去output渲染帧数的差值
    uint32_t lagged;//obs_core_video lagged帧数减去output lagged帧数的差值

    uint32_t dropped_frame; //output丢弃的帧数
    uint32_t total_frame; //output总共应该出去的帧数
    uint64_t total_bytes; //output总共应该发出去的字节数
    uint64_t dropped_frame_bytes; //output网络丢帧丢掉的字节数
    long skipped; //编码器没有参与编码的帧数
};
//获取推流质量
EXPORT struct streaming_info obs_output_get_streaming_info(struct obs_output *output);


//上报当前CDN地址推流三次发包失败
EXPORT  void obs_output_post_send_fail(struct obs_output* output);
...
typedef struct gpus_info
{
    gpu_info gpus[4];
    int cur_load_gpu_index;
    int gpu_count;
}gpus_info;
//获取GPU信息
EXPORT struct gpus_info obs_get_gpus_info();
EXPORT int obs_get_device_loss_count();
//cpu和windows版本号，因为都是从obs-windows.c拿出来，所以我们放一个结构体
typedef struct cpu_and_os_version
{
    char cpu_name[512];
    char os_version[512];
    int speed;
...
//获取cpu以及windows版本号
EXPORT struct cpu_and_os_version obs_get_cpu_and_os_version();
// ]
