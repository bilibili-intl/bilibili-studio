...
static const char *source_signals[] = {
    ...
    // bililive [
    "void screen_cast_receiver_status(ptr source, int status)",
    "void ios_screen_cast_receiver_status(ptr source, int ev_type, int val)",
    "void camera_signal(ptr source, int status)",
    "void camera_occupied_signal(ptr source, bool camera)",
    "void media_callback(ptr source, int state)",
    "void sing_identify_receiver_status(ptr source, int status)",
    // ]
	NULL,
};
			   ...
			   bool private, uint32_t last_obs_ver)
{
	...
	blog(LOG_INFO, "%ssource '%s' (%s) created", private ? "private " : "",
    //]
	     name, id);
...
static void obs_source_deferred_update(obs_source_t *source)
{
	if (source->context.data && source->info.update) {
		...
		source->info.update(source->context.data,
				    source->context.settings);
	    // bililive [
        if (source->deferred_updated_handler)
        {
            source->deferred_updated_handler(source->deferred_updated_handler_param);
        }
		// ]
			    ...
			    const struct obs_source_frame *src)
{
	...
	// bililive [
	if (dst->extra_data && dst->extra_data_op)
		(*dst->extra_data_op->destroy)(dst->extra_data);
    dst->extra_data = 0;
	dst->extra_data_op = 0;
	if (src->extra_data && src->extra_data_op)
	{
		dst->extra_data_op = src->extra_data_op;
		dst->extra_data = (*src->extra_data_op->copy)(src->extra_data);
	}
	// ]
}
...
bool obs_source_is_deferred_updated_handler_installed(obs_source_t* source)
{
    if (!obs_source_valid(source, "obs_source_is_deferred_updated_handler_installed"))
    {
        return 0;
    }

    return source->deferred_updated_handler != NULL;
}

void obs_source_install_deferred_updated_handler(obs_source_t* source, deferred_updated_callback handler,
                                                 void* param)
{
    if (!obs_source_valid(source, "obs_source_install_deferred_updated_handler"))
    {
        return;
    }

    source->deferred_updated_handler = handler;
    source->deferred_updated_handler_param = param;
}

void obs_source_uninstall_deferred_updated_handler(obs_source_t* source)
{
    if (!obs_source_valid(source, "obs_source_uninstall_deferred_updated_handler"))
    {
        return;
    }

    source->deferred_updated_handler = NULL;
    source->deferred_updated_handler_param = NULL;
}

struct obs_source_frame* obs_source_peek_frame(obs_source_t* source)
{
	struct obs_source_frame* frame = NULL;

	if (!obs_source_valid(source, "obs_source_peek_frame"))
		return NULL;
