#ifndef BILIBASE_SIGNALS_H_
#define BILIBASE_SIGNALS_H_
...
struct SlotImpl : std::enable_shared_from_this<SlotImpl<Func, Args...>>, IDisposable {
    ...
    DISABLE_COPY(SlotImpl);

    DISABLE_MOVE(SlotImpl);
...
public:
    ...
    DISABLE_COPY(SignalImpl);

    DISABLE_MOVE(SignalImpl);
    ...
    void RemoveSlot(const std::shared_ptr<SlotType>& slot)
    {
        ...
        auto it = std::find_if(slots.begin(), slots.end(),
                               [&slot](const std::shared_ptr<SlotType>& other_slot) {
