...
{
    return ViddupOutputController::GetInstance();
}

bool OutputController::IsStreamProtocolSupported(const std::string& protocol)
{
...
void OutputController::Bind(obs_proxy::OBSCoreProxy* core)
{
    DCHECK(core);
    ...
    obs_output_events.start_recording = std::bind(&OutputController::OnStartRecording, this);
    obs_output_events.stop_recording = std::bind(&OutputController::OnStopRecording, this, _1);
    obs_output_events.recording_error = std::bind(&OutputController::OnRecordingError, this, _1, _2);

    obs_output_events.start_streaming = std::bind(&OutputController::OnStartStreaming, this);
    obs_output_events.stop_streaming = std::bind(&OutputController::OnStopStreaming, this, _1);
    obs_output_events.streaming_error = std::bind(&OutputController::OnStreamingError, this, _1, _2);

    obs_output_events.streaming_feedback = std::bind(&OutputController::OnStreamingFeedback, _1);
    obs_output_events.encoded_qp_feedback = std::bind(&OutputController::OnStreamEncodedQPFeedback, _1);

    obs_output_events.post_send_failing = std::bind(&OutputController::OnPostSendFailing, this);
}

void OutputController::UnBind()
{
    auto& obs_output_events = obs_core()->GetOBSOutputService()->events();
...
bool OutputController::IsCodecReady() const
{
    return !obs_core()->GetOBSOutputService()->IsOutputActive();
}

void OutputController::StartRecording()
{
...
void OutputController::OnStartRecording()
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
...
void OutputController::OnStopRecording(const base::FilePath& video_path)
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
...
void OutputController::OnRecordingError(obs_proxy::RecordingErrorCode error_code,
...
void OutputController::StartStreaming(const secret::LiveStreamingService::StartLiveInfo& start_live_info)
{
...
void OutputController::StopStreaming()
{
...
void OutputController::OnStartStreaming()
{
...
void OutputController::OnStreamingError(obs_proxy::StreamingErrorCode error_code,
        ...
        error_code == obs_proxy::StreamingErrorCode::InvalidProtocol)
    {
            ...
            retry_count_after_colive_ < 5)
        {
            retry_timer_.Start(
                ...
                base::Bind(&OutputController::OnRetryStreamingTimer, base::Unretained(this)));
...
void OutputController::OnStopStreaming(obs_proxy::StreamingStopDetails stop_details)
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
...
void OutputController::OnStreamingFeedback(const obs_proxy::StreamingFeedbackMetrics& metrics)
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
...
void OutputController::OnStreamEncodedQPFeedback(int qp)
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
...
void OutputController::RetryStreaming() {
    if (delegate_) {
...
void OutputController::OnRetryStreamingTimer() {
    // 校验状态
    if (streaming_status_ == LiveStreamingStatus::IgnitingStreamAfterColive ||
...
void OutputController::IgniteStreamUntilAddrDetermined(bool restarted)
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
...
void OutputController::IgniteStream(bool after_colive)
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
...
void OutputController::IgniteStreamBySpecialAddr(const std::string& server_addr, const std::string& server_key, const std::string& protocol)
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
...
void OutputController::IgniteStreamUntilSpeedTest()
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
...
void OutputController::DetermineStreamingAddr()
{
    ...
    if (!new_link_mode_)
    {
        ...
        using namespace std::placeholders;
        auto callback = std::bind(&OutputController::OnDetermineStreamingAddr, this, _1, _2, _3, _4);
...
void OutputController::OnDetermineStreamingAddr(
...
void OutputController::StartStreamingInternal(
...
void OutputController::StopStreamingInternal(bool restart)
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
...
void OutputController::StreamingAnnihilated(obs_proxy::StreamingErrorCode streaming_errno,
    const obs_proxy::StreamingErrorDetails& streaming_error_details)
{
    ...
    // 上层应该在切实关播了之后再主动通知OutputController::InformLiveRoomClosed重置状态
...
void OutputController::Restart(LiveRestartMode mode)
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
...
const OutputController::StreamingAddresses& OutputController::GetStreamingAddr() const
{
    return saved_addresses_;
}

bool OutputController::IsRestartLiveStreamingSafePeriodExpires() const
{
    return last_restart_live_streaming_time_ + kRestartLiveStreamingSafePeriod < base::Time::Now();
}

void OutputController::DoRestartIfNeeded()
{
    if (restart_mode_.IsRestartStreamBitsSet())
    {
...
bool OutputController::DoRestartAfterStreamingStoppedIfNeeded() {
...
void OutputController::DoRestart()
{
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
    ...
    if (obs_core()->GetOBSOutputService()->IsOutputActive())
    {
        if (retry_restart_count++ < 100)
        {
            ...
            BililiveThread::PostDelayedTask(BililiveThread::UI,
                FROM_HERE,
                base::Bind(&OutputController::DoRestart, base::Unretained(this)),
...
void OutputController::DoRestartForColive() {
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
    ...
    if (obs_core()->GetOBSOutputService()->IsOutputActive())
    {
        BililiveThread::PostDelayedTask(BililiveThread::UI,
            FROM_HERE,
            base::Bind(&OutputController::DoRestartForColive, base::Unretained(this)),
...
int OutputController::GetOutputTotalFrames() const
{
    return obs_core_->GetOBSOutputService()->GetOutputTotalFrames();
}

int OutputController::GetOutputDroppedFrames() const
{
...
void OutputController::InformLiveRoomClosed(const OutputControllerDelegate::StreamingErrorInfos* infos)
{
    set_streaming_status(LiveStreamingStatus::Ready);
...
void OutputController::InformSpeedTestCompleted()
{
...
void OutputController::OnPostSendFailing() {
    DCHECK(BililiveThread::CurrentlyOn(BililiveThread::UI));
    //技术埋点推流：建连成功，发包失败
