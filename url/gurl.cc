...
GURL::GURL(const std::string& raw_string) : inner_url_(NULL) {
    // bililive [
    raw_string_ = raw_string;
    std::string url_string(raw_string);
    if (!url_string.empty())
    {
        // copy from url\url_util.cc: 40 kStandardURLSchemes
        static const char kURLSchemesRegexPattern[] = R"(^(http|https|file|ftp|gopher|ws|wss|filesystem)://\S+)";
        std::smatch match;
        std::regex regex(kURLSchemesRegexPattern);
        if (!std::regex_match(url_string, match, regex))
        {
...
GURL::GURL(const base::string16& raw_string) : inner_url_(NULL) {
    // bililive [
    raw_string_ = base::UTF16ToUTF8(raw_string);
    base::string16 url_string(raw_string);
    if (!url_string.empty())
    {
        // copy from url\url_util.cc: 40 kStandardURLSchemes
        static const wchar_t kURLSchemesRegexPattern[] = LR"(^(http|https|file|ftp|gopher|ws|wss|filesystem)://\S+)";
        std::wsmatch match;
        std::wregex regex(kURLSchemesRegexPattern);
        if (!std::regex_match(url_string, match, regex))
        {
            url_string = L"https://" + url_string;
        }
    }
    // ]
  is_valid_ = InitCanonical(url_string, &spec_, &parsed_);
