...
//#include "base/test/test_assistant.h"
...
// but unique non-negative integer assigned by this assistant to each test
...
bool RunTests(TestLauncherDelegate* assistant_delegate,
              int total_shards,
              int shard_index) {
  ...
  for (int i = 0; i < unit_test->total_test_case_count(); ++i) {
    ...
    for (int j = 0; j < test_case->total_test_count(); ++j) {
      ...
      if (!assistant_delegate->ShouldRunTest(test_case, test_info))
        continue;
      ...
      assistant_delegate->RunTest(test_case,
                                 test_info,
                                 base::Bind(
  ...
  assistant_delegate->RunRemainingTests();
...
int LaunchChildGTestProcess(const CommandLine& command_line,
  ...
  // Strip out gtest_repeat flag - this is handled by the assistant process.
...
int LaunchTests(TestLauncherDelegate* assistant_delegate,
                int argc,
                char** argv) {
  const CommandLine* command_line = CommandLine::ForCurrentProcess();
  ...
  int exit_code = 0;
  while (cycles != 0) {
    if (!RunTests(assistant_delegate, total_shards, shard_index)) {
      exit_code = 1;
      break;
    }
