...
bool GetAppOutputInternal(const StringPiece16& cl, std::string* output, bool hide_window) {
  ...
  start_info.dwFlags |= hide_window ? (STARTF_USESTDHANDLES | STARTF_USESHOWWINDOW) : STARTF_USESTDHANDLES;
...
bool GetAppOutput(const CommandLine& cl, std::string* output) {
  return GetAppOutputInternal(cl.GetCommandLineString(), output, false);
}

bool GetAppOutput(const StringPiece16& cl, std::string* output, bool hide_window) {
  return GetAppOutputInternal(cl, output, hide_window);
}
