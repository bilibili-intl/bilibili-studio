...
// bililive[
bool PrefValueMap::GetInteger64(const std::string& key, int64_t* value) const {
  const base::Value* stored_value = nullptr;
  return GetValue(key, &stored_value) && stored_value->GetAsInteger64(value);
}
// ]bililive
...
void PrefValueMap::SetInteger(const std::string& key, const int value) {
  // bililive[
  // SetValue(key, new base::FundamentalValue(value));
  SetValue(key, new base::FundamentalValue(int64_t(value)));
  // ]bililive
}

// bililive[
void PrefValueMap::SetInteger64(const std::string& key, int64_t value) {
  SetValue(key, new base::FundamentalValue(value));
}
// ]bililive
