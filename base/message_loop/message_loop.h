...
class BASE_EXPORT MessageLoop : public MessagePump::Delegate {
 public:
  ...
  //
  class DestructionObserverMonitor;//bilibili add [fix crash]
  class BASE_EXPORT DestructionObserver {
   public:
    virtual void WillDestroyCurrentMessageLoop() = 0;
    void SetMonitor(DestructionObserverMonitor * monitor);//bilibili add [fix crash]
  protected:
    virtual ~DestructionObserver();
  private:
      DestructionObserverMonitor *monitor_ = nullptr;
  };

  //bilibili add [fix crash]
  class DestructionObserverMonitor {
  public:
      DestructionObserverMonitor();
      DestructionObserverMonitor(DestructionObserver* dest_obs);
      void SetObserverDeleted();
      bool IsObserverDeleted();
  private:
      DestructionObserver* dest_obs_ = nullptr;
  };
 ...
 private:
  ...
  //bilibili add [fix crash]
  std::map<DestructionObserver*, DestructionObserverMonitor*> observer_monitor_map_;
