...
MessageLoop::DestructionObserver::~DestructionObserver() {
    if (monitor_)
    {
        monitor_->SetObserverDeleted();
    }
}
void MessageLoop::DestructionObserver::SetMonitor(DestructionObserverMonitor* monitor)
{
    monitor_ = monitor;
}

MessageLoop::DestructionObserverMonitor::DestructionObserverMonitor()
{
}
MessageLoop::DestructionObserverMonitor::DestructionObserverMonitor(DestructionObserver* dest_obs):
    dest_obs_(dest_obs)
{
    if (dest_obs_)
    {
        dest_obs_->SetMonitor(this);
    }
}
void MessageLoop::DestructionObserverMonitor::SetObserverDeleted()
{
    dest_obs_ = nullptr;
}
bool MessageLoop::DestructionObserverMonitor::IsObserverDeleted()
{
    return dest_obs_ == nullptr;
}
...
MessageLoop::~MessageLoop() {
  ...
  for (auto it = observer_monitor_map_.begin();it != observer_monitor_map_.end();it++)
  {
      if (it->second && it->second->IsObserverDeleted())
      {
		  destruction_observers_.RemoveObserver(it->first);
      }
  }
...
void MessageLoop::AddDestructionObserver(
  ...
  DestructionObserverMonitor* monitor = new DestructionObserverMonitor(destruction_observer);
  observer_monitor_map_[destruction_observer] = monitor;
}
...
void MessageLoop::RemoveDestructionObserver(
  ...
  auto it = observer_monitor_map_.find(destruction_observer);
  if (it != observer_monitor_map_.end())
  {
      if (it->second)
      {
          destruction_observer->SetMonitor(nullptr);
          delete it->second;
      }
      observer_monitor_map_.erase(it);
  }
}
...
//[[bililive
void MessageLoopForUI::SetWaitMsg(bool wait)
{
    pump_win()->SetWaitMsg(wait);
}
//blilive]]
