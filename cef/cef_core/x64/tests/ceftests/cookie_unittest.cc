...
void TestInvalidCookie2(CefRefPtr<CefCookieManager> manager,
                       CefRefPtr<CefWaitableEvent> event) {
  CookieVector cookies;

  CefCookie cookie;
  const char* kUrl = "https://.bilibili.com";
  CefString(&cookie.name).FromASCII("b_lsid");
  CefString(&cookie.value).FromASCII("E66B31B9_183D027E9B5");
  CefString(&cookie.domain).FromASCII(".bilibili.com");  // domain mismatch

  cookies.push_back(cookie);

  // No cookies will be set due to non canonical cookie
  SetCookies(manager, kUrl, cookies, true, event);
}
...
TEST(CookieTest, BasicInvalidCookie) {
  ...
  TestInvalidCookie2(manager, event);
}
