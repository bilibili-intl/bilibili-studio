...
// bililive[
void* ZipOpenFunc_64(void* opaque, const void* filename, int mode) {
    DWORD desired_access, creation_disposition;
    DWORD share_mode, flags_and_attributes;
    HANDLE file = 0;
    void* ret = NULL;
    ...
    base::string16 filename16 = UTF8ToUTF16((char*)filename);
    if ((filename != NULL) && (desired_access != 0)) {
        file = CreateFile(filename16.c_str(), desired_access, share_mode,
            NULL, creation_disposition, flags_and_attributes, NULL);
    }
...
// bililive[
unzFile OpenForUnzipping_64(const std::string& file_name_utf8) {
    zlib_filefunc64_def* zip_func_ptrs = NULL;
#if defined(OS_WIN)
    zlib_filefunc64_def zip_funcs;
    fill_win32_filefunc64(&zip_funcs);
    zip_funcs.zopen64_file = ZipOpenFunc_64;
    zip_func_ptrs = &zip_funcs;
#endif
