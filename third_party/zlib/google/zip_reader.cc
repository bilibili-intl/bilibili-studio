...
//[bililive
ZipReader::EntryInfo::EntryInfo(const std::string& file_name_in_zip,
    const unz_file_info64& raw_file_info)
    : file_path_(base::FilePath::FromUTF8Unsafe(file_name_in_zip)),
    is_directory_(false) {
    original_size_ = raw_file_info.uncompressed_size;
...
// bililive[
bool ZipReader::Open_64(const base::FilePath& zip_file_path) {
    DCHECK(!zip_file_);

    // Use of "Unsafe" function does not look good, but there is no way to do
    // this safely on Linux. See file_util.h for details.
    zip_file_ = internal::OpenForUnzipping_64(zip_file_path.AsUTF8Unsafe());
    if (!zip_file_) {
        return false;
    }
...
bool ZipReader::OpenCurrentEntryInZip_64() {
    DCHECK(zip_file_);

    unz_file_info64 raw_file_info = {};
    char raw_file_name_in_zip[internal::kZipMaxPath] = {};
    const int result = unzGetCurrentFileInfo64(zip_file_,
        &raw_file_info,
        raw_file_name_in_zip,
        sizeof(raw_file_name_in_zip) - 1,
        NULL,  // extraField.
    ...
    std::string file_name_in_zip = raw_file_name_in_zip;
    string16 file_name_utf16;
    if (!UTF8ToUTF16(raw_file_name_in_zip, strlen(raw_file_name_in_zip),
        &file_name_utf16)) {
        file_name_in_zip = GBKToUtf8(raw_file_name_in_zip);
    }

    current_entry_info_.reset(
        new EntryInfo(file_name_in_zip, raw_file_info));
    return true;
}
//]bililive
