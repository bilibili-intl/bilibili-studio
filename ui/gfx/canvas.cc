...
namespace gfx {
 ...
 // bililive 本文件中canvas_指针都做了空指针判断，避免崩溃
...
bool Canvas::GetClipBounds(gfx::Rect* bounds) {
  SkRect out;
  bool has_non_empty_clip;

  if (canvas_) {
       has_non_empty_clip = canvas_->getClipBounds(&out);
   }
