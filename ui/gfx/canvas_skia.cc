...
void Canvas::SizeStringInt(const base::string16& text,
  ...
  // bililive[
  if (SizeStringIntD2D(adjusted_text, font, width, height, line_height, flags)) {
    return;
  }
  // ]bililive
...
void Canvas::DrawStringWithShadows(const base::string16& text,
  ...
  if (shadows.empty())
  {
      if (DrawStringWithShadowsD2D(text, font, color, text_bounds, line_height, flags, shadows))
      {
          return;
      }
  }
