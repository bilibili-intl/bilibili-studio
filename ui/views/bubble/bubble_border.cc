...
BorderImages* GetBorderImages(BubbleBorder::Shadow shadow) {
  ...
  // bililive[
  // 下面 switch 语句中的 * scale 也在 bililive 的修改范围之内
  ...
  switch (shadow) {
    case BubbleBorder::SHADOW:
      ...
      set = new BorderImages(kShadowImages, kShadowArrows, 0, 0, 3 * scale);
      break;
    case BubbleBorder::NO_SHADOW:
    case BubbleBorder::NO_SHADOW_OPAQUE_BORDER:
      set = new BorderImages(kNoShadowImages, kNoShadowArrows, 6 * scale, 7 * scale, 4 * scale);
      break;
    case BubbleBorder::BIG_SHADOW:
      set = new BorderImages(kBigShadowImages, kBigShadowArrows, 23 * scale, 9 * scale, 2 * scale);
      break;
    case BubbleBorder::SMALL_SHADOW:
      set = new BorderImages(
          kSmallShadowImages, kSmallShadowArrows,
          5 * scale, 6 * scale, 2 * scale);
      break;
