...
void View::RemoveChildViewCancelNotifications(View* view)
{
    DoRemoveChildView(view, true, true, false, NULL,false);
}
...
bool View::enabled() const
{
    if (enabled_)
    {
        bool enable = enabled_;
        if (enable)
        {
            const views::View* p = this;
            do
            {
                p = p->parent();
                if (p)
...
//bililive[
void View::KeyEventDispatch(ui::KeyEvent* event) {
	/*OnKeyEvent(event);
	if (!event->handled() && has_children() && HasFocus()) {
		for (Views::const_reverse_iterator it(children_.rbegin());
			it != children_.rend(); ++it) {
			(*it)->KeyEventDispatch(event);
		}

	}*/
}

bool View::OnIMEMessage(const MSG event, LRESULT* result){
    return false;
}
...
bool View::OnUntranslatedIMEMessage(const MSG event, LRESULT* result){
	bool  handled = false;
	//handled = OnIMEMessage(event, result);
	//if (!handled && has_children() && HasFocus()) {
	//	for (Views::const_reverse_iterator it(children_.rbegin());
	//		it != children_.rend(); ++it) {
	//		(*it)->OnUntranslatedIMEMessage(event,result);
	//	}
	//}
	return handled;
}
...
void View::DoRemoveChildView(View* view,
                             ...
                             View* new_parent,
                             bool notifications) {// bililive [bool notifications]
  DCHECK(view);
  const Views::iterator i(std::find(children_.begin(), children_.end(), view));
  ...
  if (i != children_.end()) {
    ...
    view->PropagateRemoveNotifications(this, new_parent, notifications);
    view->parent_ = NULL;
...
void View::PropagateRemoveNotifications(View* old_parent, View* new_parent, bool notifications) {// bililive [bool notifications]
  for (int i = 0, count = child_count(); i < count; ++i)
    child_at(i)->PropagateRemoveNotifications(old_parent, new_parent, notifications);

  ViewHierarchyChangedDetails details(false, old_parent, this, new_parent);
  for (View* v = this; v; v = v->parent_)
    v->ViewHierarchyChangedImpl(true, details, notifications);
}
...
void View::BoundsChanged(const gfx::Rect& previous_bounds) {
  ...
  if (descendants_to_notify_.get()) {
    Views views_need_notify = *descendants_to_notify_;
    for (Views::iterator i(views_need_notify.begin());
      i != views_need_notify.end(); ++i) {
