...
void NativeComboboxViews::UpdateFromModel() {
  ...
  menu->set_requested_menu_position(MenuItemView::POSITION_BELOW_BOUNDS);
...
void NativeComboboxViews::ShowDropDownMenu(ui::MenuSourceType source_type) {
  ...
  dropdown_open_ = true;
  // 
  gfx::Rect arrow_bounds(this->bounds().right() - 51, 1, 50, this->bounds().height() - 2);
  SchedulePaintInRect(arrow_bounds);
