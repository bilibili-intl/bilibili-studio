...
class VIEWS_EXPORT TableView
 ...
 public:
  ...
  void SetRowHeight(int cy) { row_height_ = cy; }
  void SetTextHorizontalPadding(int padding);
  void SetIconSize(int size) { icon_size_ = size; }
  void SetBackgroundColor(SkColor clrNor, SkColor clrSel);
  void SetTextColor(SkColor clrNor, SkColor clrSel);
  //add by (sure): for tableview cell rect check
  gfx::Rect get_cell_bounds(int row, int visible_column_index) const {
      return GetCellBounds(row, visible_column_index);
  }
  // ]bililive
 ...
 private:
  ...
  // bililive[
  int text_horizontal_padding_;
  int icon_size_;
  SkColor bg_color_;
  SkColor selected_bg_color_;
  SkColor fg_color_;
  SkColor selected_fg_color_;
  // ]bililive
