...
TableView::TableView(ui::TableModel* model,
    ...
    : model_(NULL),
      ...
      text_horizontal_padding_(kTextHorizontalPadding),
      icon_size_(kImageSize),
      bg_color_(GetNativeTheme()->GetSystemColor(ui::NativeTheme::kColorId_TableBackground)),
      selected_bg_color_(GetNativeTheme()->GetSystemColor(text_background_color_id(true))),
      fg_color_(GetNativeTheme()->GetSystemColor(ui::NativeTheme::kColorId_TableText)),
      selected_fg_color_(GetNativeTheme()->GetSystemColor(selected_text_color_id(true))),
...
void TableView::OnPaint(gfx::Canvas* canvas) {
  ...
  for (int i = region.min_row; i < region.max_row; ++i) {
    ...
    // bililive[
    gfx::Rect row_bound = GetRowBounds(i);
    canvas->DrawLine(row_bound.bottom_left(), row_bound.bottom_right(), selected_bg_color_);
    // ]bililive
    if (is_selected) {
        canvas->FillRect(row_bound, selected_bg_color_);
    ...
    for (int j = region.min_column; j < region.max_column; ++j) {
      ...
      int text_x = text_horizontal_padding_ + cell_bounds.x();

      // Provide space for the grouping indicator, but draw it separately.
      if (j == 0 && grouper_)
          text_x += kGroupingIndicatorSize + text_horizontal_padding_;
      ...
      if (j == 0 && table_type_ == ICON_AND_TEXT) {
        ...
        if (!image.isNull()) {
          int image_x = GetMirroredXWithWidthInView(text_x, icon_size_);
          canvas->DrawImageInt(
              image, 0, 0, image.width(), image.height(),
              image_x,
              cell_bounds.y() + std::round((cell_bounds.height() - image.height()) * 1.0f / 2),
              image.width(), image.height(), true);

          //// 为使图片居中显示于chrome框架定死的kImageSize*kImageSize区域中，并不会扩大图标的显示范围又使图标显示效果最佳
          //float fwh = (float)((float)image.width() / (float)image.height());
          //int cx = std::min(image.width(), icon_size_);
          //int cy = round(cx / fwh);
          //cy = std::min(cy, icon_size_);
          //cx = round(cy * fwh);
          //canvas->DrawImageInt(
          //    image, 0, 0, image.width(), image.height(),
          //    image_x + (icon_size_ - cx) / 2,
          //    cell_bounds.y() + (cell_bounds.height() - cy) / 2,
          //    cx, cy, true);
        }
        text_x += icon_size_ + kTextHorizontalPadding;
      }
      if (text_x < cell_bounds.right() - kTextHorizontalPadding)
      {
        canvas->DrawStringInt(
            model_->GetText(model_index, visible_columns_[j].column.id), font_,
            is_selected ? selected_fg_color_ : fg_color_,
  ...
  const int group_indicator_x = GetMirroredXInView(GetCellBounds(0, 0).x() +
      text_horizontal_padding_ + kGroupingIndicatorSize / 2);
...
void TableView::AdjustCellBoundsForText(int visible_column_index,
  ...
  int text_x = text_horizontal_padding_ + bounds->x();
...
void TableView::UpdateVisibleColumnSizes() {
  ...
  if (table_type_ == ICON_AND_TEXT && header_)
    first_column_padding += icon_size_ + kTextHorizontalPadding;
  if (grouper_)
...
void TableView::SetBackgroundColor(SkColor clrNor, SkColor clrSel)
{
    bg_color_ = clrNor;
    selected_bg_color_ = clrSel;
}

void TableView::SetTextColor(SkColor clrNor, SkColor clrSel)
{
    fg_color_ = clrNor;
    selected_fg_color_ = clrSel;
}

void TableView::SetTextHorizontalPadding(int padding)
{
    DCHECK(padding >= 0);
    text_horizontal_padding_ = padding;
}
