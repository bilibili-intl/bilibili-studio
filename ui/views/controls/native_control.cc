...
#define REFLECT_MSG_TO_TARGET(target) \
	{ \
		bHandled = TRUE; \
        if(target != NULL)\
		    lResult = target->OnReflectMsg(uMsg, wParam, lParam, bHandled); \
		if(bHandled) \
			return TRUE; \
	}
...
LRESULT NativeControl::OnReflectMsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
  bHandled = false;
  return 0;
}
