...
void Textfield::UpdateAllProperties() {
  if (native_wrapper_) {
    ...
    // bililive[
    ((NativeTextfieldViews*)native_wrapper_)->GetRenderText()->SetPasswordReplacementChar(obscured_ochar_);
    // ]bililive
  }
}
...
SkColor Textfield::GetCursorColor() const {
  if (!use_default_cursor_color_)
