...
void NativeWidgetWin::InitNativeWidget(const Widget::InitParams& params) {
  ...
  gfx::Rect pixel_bounds = params.bounds;
  // ]bililive
...
void NativeWidgetWin::ShowMaximizedWithBounds(
  ...
  gfx::Rect pixel_bounds = restored_bounds;
  // ]bililive
...
// bililive[
void NativeWidgetWin::SetDisableRedrawWhenDidProcessMsg(bool disable) {
    message_handler_->set_disable_redraw_when_did_process_msg(disable);
}
// ]bililive
...
void NativeWidgetWin::HandleCreate() {
  ...
  if (View::get_use_acceleration_when_possible()) {
    {
      CRect window_rect;
      GetClientRect(GetNativeView(), &window_rect);
...
void NativeWidgetWin::HandleClientSizeChanged(const gfx::Size& new_size) {
  ...
  gfx::Size size_in_dip = new_size;
  // ]bililive
  if (compositor_.get())
    compositor_->WidgetSizeChanged(size_in_dip);
...
bool NativeWidgetWin::HandleIMEMessage(UINT message,
	...
	msg.lParam = l_param;
	//// bililive[
	//if (GetWidget()->OnUntranslatedIMEMessage(msg, result)) {
	//	return true;
	//}
	////]bililive
...
bool NativeWidgetWin::HandlePaintAccelerated(const gfx::Rect& invalid_rect) {
  ...
  gfx::Rect dpi_rect = invalid_rect;
  // ]bililive
...
void NativeWidgetWin::SetInitParams(const Widget::InitParams& params) {
  ...
  // bililive[
  message_handler_->set_use_new_translucent(params.opacity == Widget::InitParams::NEW_TRANSLUCENT_WINDOW);
  // ]bililive
}
