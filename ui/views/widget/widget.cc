...
Widget::InitParams::InitParams(Type type)
    : type(type),
      ...
      desktop_root_window_host(NULL),
      top_level(false),
      remove_taskbar_icon(type == TYPE_POPUP),
      layer_type(ui::LAYER_TEXTURED),
      context(NULL) {
}
...
bool Widget::OnNativeWidgetPaintAccelerated(const gfx::Rect& dirty_region) {
  ...
  if (GetRootView()->layer()) {
    ...
    if (layer_transform != GetRootView()->GetTransform()) {
      ...
      gfx::RectF layer_bounds_f = layer_bounds;
      layer_transform.TransformRect(&layer_bounds_f);
      layer_bounds.set_origin(gfx::Point(layer_bounds_f.x(), layer_bounds.y()));
      layer_bounds.set_size(gfx::Size(layer_bounds_f.width(), layer_bounds_f.height()));
...
void Widget::OnKeyEvent(ui::KeyEvent* event) {
 ...
 /* if (!event->handled()) {
	  ...
	  if (root_view)
		  root_view->KeyEventDispatch(event);
  }*/
...
bool Widget::OnUntranslatedIMEMessage(const MSG event, LRESULT* result) {
	View* root_view = GetRootView();
    bool  handled = false;
    if (root_view)
        handled = root_view->OnUntranslatedIMEMessage(event, result);
	return handled;
}
//]bililive
