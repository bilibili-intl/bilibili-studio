    ...
    bool has_ellipsis)
{
    ...
    const string16 ellipsis = string16(L"...");
    StringSlicer slicer(text, ellipsis, false);
    ...
    for (guess = (lo + hi) / 2; lo <= hi; guess = (lo + hi) / 2) {
        // We check the length of the whole desired string at once to ensure we
        // handle kerning/ligatures/etc. correctly.
        const string16 cut = slicer.CutString(guess, has_ellipsis);
        const int guess_length = font.GetStringWidth(cut);
        if (guess_length > available_pixel_width) {
            if (guess <= 1) {//修复guess为0是引起崩溃的bug
                break;
            }
            hi = guess - 1;
        }
    ...
    return slicer.CutString(guess, has_ellipsis);
}
// ]bililive
    ...
    bool has_ellipsis)
{
    ...
    if (length == 0) return string16();

    string16 result;
    // ICU 的迭代器，可处理代理对和扩展字素群
    base::i18n::BreakIterator iter(text, base::i18n::BreakIterator::BREAK_CHARACTER);
    if (iter.Init()) {
        bool not_at_end;
        size_t index = 0;
        while (not_at_end = iter.Advance(), not_at_end && index < length) {
            result.append(iter.GetString());
            ++index;
        }
        if (not_at_end && has_ellipsis) {
            result.append(L"...");
        }
    } else {
        // ICU 出现错误，将只处理代理对
        ...
        U16_FWD_N(text, index, text_length, length);
        result = text.substr(0, index);
        if (index < text_length && has_ellipsis) {
            result.append(L"...");
        }
    }
    return result;
