...
// bililive[
// 将 UTF-16 字符串 |text| 截取指定的字符数量 |length|。
// 代理对 (Surrogate Pairs) 已正确处理。
// 字素群 (Grapheme Cluster) 在 ICU 初始化成功后也将正确处理，否则将被忽略，不作为字符边界。
// 关于代理对，可参见 Unicode 文档：https://www.unicode.org/standard/standard.html 2.5 节 [UTF-16]
// 关于字素群，可参见 Unicode 文档，2.1 节 [Text Elements, Characters, and Text Processes]
UI_EXPORT string16 ElideText(
    const string16& text,
    size_t length,
    bool has_ellipsis);
