...
// bililive[
UI_EXPORT void CenterAndSizeWindowEx(HWND parent, HWND window, const gfx::Size& pref)
{
    DCHECK(window && pref.width() > 0 && pref.height() > 0);
    ...
    gfx::Rect parent_bounds(center_bounds);
    ...
    window_bounds.left = parent_bounds.x() + (parent_bounds.width() - pref.width()) / 2;
    window_bounds.right = window_bounds.left + pref.width();

    window_bounds.top = parent_bounds.y() + (parent_bounds.height() - pref.height()) / 2;
    window_bounds.bottom = window_bounds.top + pref.height();
