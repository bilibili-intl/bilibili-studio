...
#include "layer.h"
#include "test_compositor.h"
#include "test_compositor_host.h"
...
class LayerWithRealCompositorTest : public testing::Test {
 public:
  ...
  Layer* CreateNoTextureLayer(const gfx::Rect& bounds) {
    Layer* layer = CreateLayer(ui::LAYER_NO_TEXTURE);
    layer->SetBounds(bounds);
    return layer;
  }

  gfx::Canvas* CreateCanvasForLayer(const Layer* layer) {
    return new gfx::Canvas(layer->bounds().size(), ui::SCALE_FACTOR_100P, false);
  }
  ...
  void PaintColorToLayer(Layer* layer, SkColor color) {
    ...
    canvas->FillRect(
      ...
      color);
    layer->SetCanvas(*canvas->sk_canvas(), layer->bounds().origin());
  }
...
class TestLayerDelegate : public LayerDelegate {
 public:
  ...
  virtual void OnPaintLayer(gfx::Canvas* canvas) OVERRIDE {
    SkBitmap contents = canvas->ExtractImageRep().sk_bitmap();
    paint_size_ = gfx::Size(contents.width(), contents.height());
    canvas->FillRect(gfx::Rect(0, 0,
                        contents.width(),
                        contents.height()), colors_.at(color_index_));
...
class LayerWithDelegateTest : public testing::Test {
 public:
  ...
  virtual Layer* CreateLayer(ui::LayerType type) {
  ...
  void PaintColorToLayer(Layer* layer, SkColor color) {
    scoped_ptr<gfx::Canvas> canvas(CreateCanvasForLayer(layer));
    canvas->FillRect(layer->bounds(), color);
    layer->SetCanvas(*canvas->sk_canvas(), layer->bounds().origin());
  }
...
class LayerWithNullDelegateTest : public LayerWithDelegateTest {
 public:
  ...
  Layer* CreateLayer (ui::LayerType type) OVERRIDE {
...
TEST_F(LayerWithNullDelegateTest, HoleWithNinetyDegreeTransforms) {
  ...
  for (int i = -4; i <= 4; ++i) {
    ...
    t.TransformRect(&target_rect_f);
    target_rect.SetRect(target_rect_f.x(), target_rect_f.y(), target_rect_f.width(), target_rect_f.height());
...
TEST_F(LayerWithNullDelegateTest, NoCompositor) {
  scoped_ptr<Layer> l1(CreateLayer(ui::LAYER_NO_TEXTURE));
  scoped_ptr<Layer> l11(CreateLayer(ui::LAYER_TEXTURED));
  scoped_ptr<Layer> l12(CreateLayer(ui::LAYER_NO_TEXTURE));
  scoped_ptr<Layer> l121(CreateLayer(ui::LAYER_TEXTURED));
  scoped_ptr<Layer> l122(CreateLayer(ui::LAYER_TEXTURED));
...
TEST_F(LayerWithNullDelegateTest, Visibility) {
  scoped_ptr<Layer> l1(new Layer(NULL, ui::LAYER_TEXTURED));
  scoped_ptr<Layer> l2(new Layer(NULL, ui::LAYER_TEXTURED));
  scoped_ptr<Layer> l3(new Layer(NULL, ui::LAYER_TEXTURED));
  l1->Add(l2.get());
  l2->Add(l3.get());
